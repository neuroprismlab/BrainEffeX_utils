...
#' # Example usage
#' # plot_density(pd)
plot_overlapping_densities <- function(plot_data_list, save = FALSE, out_path = 'output', file_name = 'plot') {
  ...
  if (save) {
    ...
    png(out_name, width = 5.5, height = 5, res = 300, units = "in")
  }
  
  
  # initialize
  par(mar=c(3, 4, 5, 2))
  p <- ggplot() + theme_minimal() + labs(title = "Overlapping Density Plots", x = "Effect Size", y = "Density") + xlim(-0.6, 0.6)
  
  # iterate over each dataset
  for (i in seq_along(plot_data_list)) {
    plot_data <- plot_data_list[[i]]$data$cons_estimate
    # Add a density plot for each dataset
    p <- p + geom_density(data = data.frame(value = plot_data_list[[i]]$data$cons_estimate), aes(x = value), fill = i, color = i, alpha = 0.1)
    
    # extract data
    # plot_data = plot_data_list[[i]]$data
    # summary_info = plot_data_list[[i]]$summary_info
    # mv_data = plot_data_list[[i]]$mv_data
    # study_details = plot_data_list[[i]]$study_details
   }
