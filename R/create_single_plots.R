...
#' Single spatial effect size plot
#'
...
#' @param do_minimal_title Logical indicating whether to use a minimal title. Default is FALSE.
#' 
#' @return A ggplot object representing the spatial effect size plot.
#' @export
#'
#' @examples
#' # Example usage
...
plot_one_spatial <- function(study_name, v_data, estimate = 'd', motion_method = 'none', pooling_method = 'none', add_description = TRUE, do_minimal_title = FALSE) {

  plot_info <- list(grouping_var = 'category', group_level = v_data$study[v_data$study$name == study_name,"category"], ref = v_data$study[v_data$study$name == study_name,"ref"])

  combo_name = paste0('pooling.', pooling_method, '.motion.', motion_method, '.mv.none')

  prepped <- prep_data_for_spatial_plot(data = v_data$data[[study_name]], brain_masks = v_data$brain_masks[[study_name]], study_details = v_data$study[v_data$study$name == study_name, ], 
                               combo_name = paste0('pooling.', pooling_method, '.motion.', motion_method, '.mv.none'), 
                               mv_combo_name = paste0('pooling.', pooling_method, '.motion.', motion_method, '.mv.multi'),
                               estimate = estimate, plot_info = plot_info)
